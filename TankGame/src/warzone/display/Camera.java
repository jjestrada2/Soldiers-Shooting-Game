...
public class Camera {
    ...
    private static  final int SPLIT_SCREEN_W = (GameConstants.GAME_SCREEN_WIDTH/2);
    ...
    public void drawSplitScreen(BufferedImage world) {
        int x = (int) tank.getX();
        int y = (int) tank.getY();
        splitScreen = world.getSubimage(this.checkBorderX(x), this.checkBorderY(y), SPLIT_SCREEN_W, GameConstants.GAME_SCREEN_HEIGHT - HUD_LIMIT);
    }
    ...
    private int checkBorderY(int y) {
        ...
        } else if(y > GameConstants.GAME_WORLD_HEIGHT - GameConstants.GAME_SCREEN_HEIGHT / 2 + HUD_LIMIT) {
            y = GameConstants.GAME_WORLD_HEIGHT - GameConstants.GAME_SCREEN_HEIGHT / 2 + HUD_LIMIT;
        }
        y -= GameConstants.GAME_SCREEN_HEIGHT / 2;
        return y;
    }
